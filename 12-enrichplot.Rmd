...
The `emapplot` function also supports results obtained from `compareCluster` 
function of `clusterProfiler` package.
...
emapplot(xx)
```
The number of circles in the bottom left corner can be adjusted using the `legend_n` parameter.
```{r fig.width=12, fig.height=10}
emapplot(xx,legend_n=2)
```
Proportion of clusters in the pie chart can be adjusted using the `pie` parameter,
when `pie="count"`, the proportion of clusters in the pie chart is determined by
the number of genes.

```{r fig.width=12, fig.height=10}
emapplot(xx,pie="count")
```
The pie_scale parameter can be used to resize nodes.
```{r fig.width=12, fig.height=10}
emapplot(edo, pie_scale=2)
```
```{r fig.width=12, fig.height=10}
emapplot(xx,pie="count", pie_scale=1.5)
```
You can use the `layout` parameter to adjust the layout of the pie chart.
